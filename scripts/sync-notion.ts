...
const LINKEDIN_DB_ID = process.env.LINKEDIN_DB_ID || "";
...
async function calculateSprintTime(branch: string) {
  try {
    const firstCommit = (await git.raw(["log", branch, "--reverse", "--format=%ct", "--max-count=1"])).trim();
    const lastCommit = (await git.raw(["log", branch, "--format=%ct", "--max-count=1"])).trim();
...
/* ---------------------------------------------------------
 * 💬 Criação de rascunho de post (LinkedIn)
 * --------------------------------------------------------- */
...
async function createLinkedInPost(sprintNumber: number) {
  ...
  try {
    ...
    const sprintName =
      (sprint as any).properties?.Sprint?.title?.[0]?.plain_text || `Sprint ${sprintNumber}`;
    const objetivo =
      (sprint as any).properties?.["Objetivo Principal"]?.rich_text?.[0]?.plain_text || "";
...
#VibeIntel #BuildInPublic #TypeScript #AI #DevTools #Automation #SoftwareEngineering`;
    ...
    await notion.pages.create({
      ...
      properties: {
        ...
        Tipo: { select: { name: "Milestone" } },
        Status: { status: { name: "Rascunho" } },
        "Data Publicação": {
          date: { start: new Date().toISOString().split("T")[0] },
        },
        "Sprint Relacionada": { relation: [{ id: pageId }] },
        Hashtags: {
          multi_select: [
            { name: "VibeIntel" },
            { name: "BuildInPublic" },
    ...
    console.log(`📱 Post LinkedIn criado (rascunho) para Sprint ${sprintNumber}`);
  } catch (err) {
...
(async () => {
  try {
    if (eventName === "push") {
      ...
      if (prMerged) {
        await updateSprintStatus(sprintNumber, "Concluída");
        await updateSprintDuration(sprintNumber, branch);
        await createLinkedInPost(sprintNumber);
      } else {
        await updateSprintStatus(sprintNumber, "Em Testes");
      }
    }
  } catch (error: any) {
